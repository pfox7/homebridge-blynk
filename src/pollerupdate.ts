...
export class Poller {
	...
	updateAccessory(accessory) {
		for (let i = 0; i < this.platform.updateSubscriptions.length; i++) {
			...
			let params = service.subtype.split("-"); // params[0]: name, params[1]: widget, params[2]: pin, params[3]: mode, params[4]: token
			let name = params[0];
			let widget = params[1];        
...
export function getBlynkvalue(name, widget, pinString, token, mode, callback, characteristic, Characteristic, platform) {
